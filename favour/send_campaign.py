...
<p>Beckers Pumps has a proposal for a partnership on a procurement project that I want you to attend to as soon as possible.</p>
<p>A reply at your earliest convenience would be much appreciated.</p>"""
        ...
        if sig_split != -1:
            final_html = custom_body + "<br>" + base_template[sig_split:]
        else:
            final_html = custom_body + "<br>" + base_template
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 send_campaign.py leads.csv")
    else:
        send_campaign(sys.argv[1], "email_template.html")
