...
def send_campaign(csv_file, template_file):
    ...
    print(f"Starting Vac-U-Max campaign to {total_leads} leads...")
    print("-" * 50)
    
    for i, row in enumerate(rows, 1):
        email_addr = row.get("Email Address", "").strip()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 send_vacumax_campaign.py leads.csv")
    else:
        send_campaign(sys.argv[1], "vacumax_email_template.html")
